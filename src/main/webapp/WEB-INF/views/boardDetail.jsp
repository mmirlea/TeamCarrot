...
<link
	...
	rel="stylesheet">
<link href="<c:url value='/resources/css/boardDetailStyle.css'/>"
	rel="stylesheet" />
<script
	...
	<form class="wrap" id="form" enctype="multipart/form-data">
		...
		<div class="container">
			<div class="divTitle" id="divTitle">
				...
				<div class="hTitle">
					<input type="text" id="b_title"
						name="${menu == 'board' ? 'b_title' : 'p_title'}"
						class="spanTitle" placeholder="제목을 입력하세요"
				...
				<c:if test="${menu eq 'board' }">
					...
					<div class="divCategory">
						<select id="b_cate" name="b_cate" value="${boardDTO.b_cate}"
							class="boardCategory">
				...
				<c:if test="${menu eq 'product' }">
					<!-- 중고 -->
					<div class="divPrice">
						<span class="spPrice"> <i class="fa-solid fa-won-sign"></i>
							&nbsp; <input type="text" name="p_price"
							value="${productDTO.p_price}"> &nbsp; <input
							type="checkbox" name="chkShare" id="chkShare"> <label
							for="chkShare">나눔</label> &nbsp; <input type="checkbox"
							name="p_negoyn" id="chkProposal"
							${productDTO.p_negoyn == "Y" ? "checked" : "" }> <label
							for="chkProposal">가격제안받기</label>
					...
					<div class="divCategory">
						<select name="p_cate" value="${productDTO.p_cate}"
			...
			<div class="divContent">
				...
				<textarea class="spContent" id="b_content"
					name="${menu == 'board' ? 'b_content' : 'p_content'}"><c:out
						value='${menu == "board" ? boardDTO.b_content : productDTO.p_content }' /></textarea>
			</div>
		</div>
		<!--.container-->
	</form>
